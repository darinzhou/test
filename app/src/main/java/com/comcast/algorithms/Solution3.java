...
//            System.out.print(l + "," + r + " ");
    ...
    public static List<String> getPatterns(String input, Map<String, List<Character>> map) {
        List<String> result = new ArrayList<>();
        if (input == null || map == null || input.isEmpty() || map.isEmpty()) {
            return result;
        }

        // find all key groups, keys in each group can just combined as input string
        List<List<String>> keyGoups = findKeyGroups(input, map);

        for (List<String> keyGroup : keyGoups) {
            int si = 0;
            StringBuilder one = new StringBuilder();
            findPatterns(map, keyGroup, si, one, result);
        }

        return result;
    }

    private static List<List<String>> findKeyGroups(String input, Map<String, List<Character>> map) {
        // find all substring groups, strings each of which just combined as input string
        List<List<String>> keyGroups = findAllSubtringsCombinedAsInput(input);

        // remove groups with strings not in map's ket set
        for (int i = 0; i < keyGroups.size(); ++i) {
            for (String s : keyGroups.get(i)) {
    ...
    private static List<List<String>> findAllSubtringsCombinedAsInput(String input) {
        List<List<String>> result = new ArrayList<>();

        for (int i = 1; i < input.length(); ++i) {
            String current = input.substring(0, i);
            String other = input.substring(i);

            List<List<String>> substringsFindInOther = findAllSubtringsCombinedAsInput(other);

            for (List<String> ss : substringsFindInOther) {
    ...
    private static void findPatterns(Map<String, List<Character>> map, List<String> keyGroup, int si,
                                     StringBuilder one, List<String> result) {
        if (keyGroup.size() == si) {
            result.add(one.toString());
            return;
        }

        List<Character> letters = map.get(keyGroup.get(si));
        for (int i=0; i<letters.size(); ++i) {
            one.append(letters.get(i));
            findPatterns(map, keyGroup, si+1, one, result);
            one.deleteCharAt(one.length()-1);
        }
    }

    public static int moveNonZeros(int[] a) {
        if (a == null || a.length == 0) {
            return -1;
        }
        int i = 0;
    ...
    public static boolean isFlipable(int[] a) {
        if (a == null || a.length == 0) {
            return true;
        }
        ...
        int j = a.length - 1;
        while (i <= j) {
            if (!map.containsKey(a[i]) || map.get(a[i]) != a[j]) {
                return false;
            }
            i++;
            j--;
    ...
    public static List<String> getFlipables(int k) {
        if (k == 0) {
            return null;
        }
        ...
        Map<Integer, Integer> mapMatchSelf = new HashMap<>();
        mapMatchSelf.put(0, 0);
        mapMatchSelf.put(1, 1);
        mapMatchSelf.put(8, 8);
        ...
        StringBuilder one = new StringBuilder();
        List<StringBuilder> allCombinationsForLeftHalf = new ArrayList<>();
        allCombinations(source, half, 0, one, allCombinationsForLeftHalf);

        // build results
        List<String> flipables = new ArrayList<>();
        for (StringBuilder sb : allCombinationsForLeftHalf) {
            String lh = sb.toString();
            StringBuilder rhSb = new StringBuilder(sb).reverse();
            for (int i = 0; i < rhSb.length(); ++i) {
                int ichar = rhSb.charAt(i) - '0';
                int iflip = map.get(ichar);
                char flipChar = (char)(iflip + '0');
                rhSb.setCharAt(i, flipChar);
            }
            String rh = rhSb.toString();

            if (k % 2 == 0) {
                flipables.add(lh + rh);
            } else {
                for (Integer key : mapMatchSelf.keySet()) {
                    flipables.add(lh + key + rh);
                }
            }
        }
    ...
    public static void allCombinations(StringBuilder source, int n, int si, StringBuilder one,
                                       List<StringBuilder> result) {
        if (one.length() == n) {
            result.add(new StringBuilder(one));
            return;
        }

        for (int i=si; i<source.length(); ++i) {
            one.append(source.charAt(i));
            allCombinations(source, n, i+1, one, result);
            one.deleteCharAt(one.length()-1);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        List<String> ls = getFlipables(2);

        boolean flipable = isFlipable(new int[]{1,6,9,1});
        boolean flipable2 = isFlipable(new int[]{1,7,1});

        int[] a = new int[]{1,0,3,4,5,6,0,0,9,0};
        int n = moveNonZeros(a);
        ...
        List<String> aps = getPatterns("123", map);

        int[] aa = new int[]{-1, 14, 10, 8, 1, 12, 5, 6, 2, 15, -2, 3, 11, 13, 9, 7};
